...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<tSearchParams> }) {
  ...
  const perPage = (ALLOWED_PAGE_SIZES as readonly number[]).includes(requestedPerPage) ? requestedPerPage : ALLOWED_PAGE_SIZES[0];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-8">
          ...
          <div key={p.id} className="rounded-xl border p-4">
            ...
            <div className="mt-3 flex gap-2">
              {p.stock > 0 ? (
                ...
                <button className="h-11 px-4 rounded-md border text-sm opacity-60 cursor-not-allowed" disabled>Indisponible</button>
              )}
      ...
      <div className="flex items-center justify-center gap-2 mt-8">
        <a className="px-3 py-2 rounded-md border text-sm disabled:pointer-events-none disabled:opacity-50" href={page > 1 ? makeUrl(1) : undefined}>
          «
        </a>
        <a className="px-3 py-2 rounded-md border text-sm disabled:pointer-events-none disabled:opacity-50" href={page > 1 ? makeUrl(page - 1) : undefined}>
          Précédent
        </a>
        ...
        <a className="px-3 py-2 rounded-md border text-sm disabled:pointer-events-none disabled:opacity-50" href={page < totalPages ? makeUrl(page + 1) : undefined}>
          Suivant
        </a>
        <a className="px-3 py-2 rounded-md border text-sm disabled:pointer-events-none disabled:opacity-50" href={page < totalPages ? makeUrl(totalPages) : undefined}>
          »
        </a>
      </div>
    </div>
