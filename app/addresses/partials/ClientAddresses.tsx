            ...
            {addresses.map((a) => (
              <li key={a.id} className="rounded-md border p-3 text-sm">
                <div className="font-medium flex items-center gap-2">
                  {a.name}
                  {a.isDefault ? (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-zinc-100 border">
                      Par défaut
                    </span>
                  ) : null}
                </div>
                ...
                <div className="mt-2 flex gap-2">
                  <button className="rounded-md border px-3 py-1" onClick={() => setForm({ ...a })}>
                    Éditer
                  </button>
                  <button
                  ...
                  {!a.isDefault ? (
                    <button
                      className="rounded-md border px-3 py-1"
                      onClick={() => setForm({ ...a, isDefault: true })}
                    >
                      Définir par défaut
                    </button>
                  ) : null}
      ...
      <div>
        ...
        <form onSubmit={saveAddress} className="space-y-3 text-sm">
          ...
          <div className="flex gap-2">
            <button type="submit" className="rounded-md border px-4 py-2" disabled={saving}>
            ...
            {form?.id ? (
              <button
                ...
                onClick={() => setForm({ isDefault: false })}
              >
                Annuler
              </button>
            ) : null}
