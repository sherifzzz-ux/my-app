...
export function AdminNewsletter() {
	...
	const toggleSubscriberStatus = async (subscriberId: string, currentStatus: boolean) => {
		try {
			...
			if (error) throw error;
			setSubscribers(subscribers.map(sub => sub.id === subscriberId ? { ...sub, is_active: !currentStatus } : sub));
	...
	const exportSubscribers = () => {
		...
		toast({ title: "Succès", description: "Liste des abonnés exportée" });
	};
				...
				<Card>
					<CardContent className="pt-6">
						<div className="flex items-center justify-between">
							...
							<Badge variant="secondary" className="bg-green-100 text-green-800">Actifs</Badge>
						</div>
					</CardContent>
				</Card>
				...
				<Card>
					<CardContent className="pt-6">
						<div className="flex items-center justify-between">
							...
							<Badge variant="secondary" className="bg-red-100 text-red-800">Inactifs</Badge>
						</div>
					</CardContent>
				</Card>
			</div>
				...
				<CardContent>
					...
					<div className="space-y-4">
						{filteredSubscribers.map((subscriber) => (
							<div key={subscriber.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
								<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
									<div className="flex-1">
										...
										<div className="text-sm text-muted-foreground space-y-1">
											...
											<p>Abonné le: {new Date(subscriber.subscribed_at).toLocaleDateString('fr-FR')}</p>
										</div>
									</div>
									...
									<div className="flex gap-2">
										...
										>
											{subscriber.is_active ? (<><UserX className="h-4 w-4 mr-2" />Désactiver</>) : (<><Mail className="h-4 w-4 mr-2" />Activer</>)}
										</Button>
									</div>
								</div>
							</div>
